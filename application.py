...
def main():
    ...
    if not status_df.empty:
        # Add color coding only for Status column
        def highlight_status(row):
            styles = []
            for col in row.index:
                if col == 'Status':
                    # Set background color based on status
                    bg_color = '#C8E6C9' if row[col] == 'Completed' else (  # Light green
                        '#FFCDD2' if row[col] == 'Failed' else '#FFFFFF'     # Light red
                    )
                    styles.append(f'background-color: {bg_color}; color: black')
                else:
                    # White background with black text for other columns
                    styles.append('background-color: #FFFFFF; color: black')
            return styles

        styled_df = status_df.style.apply(highlight_status, axis=1, subset=status_df.columns)
