...
class JiraTicketAutomation:
    ...
    def categorize_email(self, email):
        """Categorize email with improved pattern matching."""
        subject = email.subject.lower()
        
        # Skip notifications first
        ...
        ticket_refs = re.findall(r'[A-Z]+-\d+', email.subject)
    ...
    def update_jira_ticket(self, ticket_key, email):
        ...
        try:
            ...
            comment = f"Update from {email.sender}:\n{email.body}"
...
# Add these new methods to the JiraTicketAutomation class

def delete_jira_ticket(self, ticket_key, email_data):
    """Delete a Jira ticket with validation"""
    try:
        # Security check - verify sender is authorized
...
# Update the categorize_email method
def categorize_email(self, email):
...
# Update the process_new_email method
def process_new_email(self, email_msg):
    """Updated with ticket management commands"""
    email_id = str(uuid.uuid4())
    try:
        email_data = self.email_processor.parse_email(email_msg)
        self.dashboard.update_status(email_id, "Processing", "Email received")
        ...
        if isinstance(category, tuple):
            ...
            if action == 'delete':
                if self.delete_jira_ticket(ticket_key, email_data):
                    self.dashboard.update_status(email_id, "Completed", f"Deleted {ticket_key}")
                else:
                    self.dashboard.update_status(email_id, "Failed", f"Delete failed {ticket_key}")
                return
                    
            elif action == 'update':
                if self.update_jira_issue(ticket_key, email_data):
                    self.dashboard.update_status(email_id, "Completed", f"Updated {ticket_key}")
                else:
                    self.dashboard.update_status(email_id, "Failed", f"Update failed {ticket_key}")
                return
        
        # Existing processing logic for other categories
            if category in ['notification', 'spam']:
            ...
            if isinstance(category, tuple):  # Existing ticket
